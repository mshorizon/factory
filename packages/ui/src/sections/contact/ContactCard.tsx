...
import type { ContactCardProps, SelectField } from "./types";
...
export function ContactCard({
  ...
  return (
    <div className={cn("grid lg:grid-cols-2 gap-spacing-2xl items-center", className)}>
      {/* Left — info */}
      <ScrollReveal delay={0.1} direction="left" distance={30}>
        <div className="flex flex-col gap-spacing-2xl">
          {/* Title block */}
          <div className="flex flex-col gap-spacing-md">
            <span className="w-8 h-0.5" style={{ backgroundColor: "var(--foreground)", opacity: 0.5 }} />
            {title && (
              <h2
            ...
            {subtitle && (
              <p className="text-muted text-base leading-relaxed max-w-xs" data-field="header.subtitle">
                {subtitle}
              </p>
            )}
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-spacing-lg">
            <div className="grid sm:grid-cols-2 gap-spacing-lg">
              <div className="space-y-spacing-xs">
                <Label htmlFor="card-name" className="text-foreground text-sm">
                  {form?.nameLabel || "Name"}
                </Label>
                <Input
                  type="text"
                  ...
                  className="bg-secondary/50 border-transparent focus-visible:border-border/40 text-foreground"
                />
              </div>
              <div className="space-y-spacing-xs">
                <Label htmlFor="card-email" className="text-foreground text-sm">
            ...
            {(form?.selectFields ?? []).map((field: SelectField) => (
              <div key={field.name} className="space-y-spacing-xs">
                {field.label && (
                  <Label htmlFor={`card-${field.name}`} className="text-foreground text-sm">
                    {field.label}
                  </Label>
                )}
                <select
                  ...
                  name={field.name}
                  disabled={status === "loading"}
                  className="w-full min-w-0 rounded-md border border-border/20 bg-secondary/50 text-foreground px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring"
            ...
            <div className="space-y-spacing-xs">
              <Label htmlFor="card-message" className="text-foreground text-sm">
