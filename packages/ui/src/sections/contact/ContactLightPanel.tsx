...
import { MapPin, Phone, Mail, MessageCircle, CheckCircle, AlertCircle } from "lucide-react";
...
import type { ContactLightPanelProps, SelectField } from "./types";

export function ContactLightPanel({
  title,
  subtitle,
  form,
  info,
  ...
  return (
    <div className={cn("grid lg:grid-cols-[5fr,7fr] gap-spacing-2xl items-center", className)}>
      ...
      <ScrollReveal delay={0.2} direction="right" distance={30}>
        <div className="bg-background border border-border/30 rounded-2xl shadow-md p-spacing-2xl">
          <form onSubmit={handleSubmit} className="space-y-spacing-lg">
            {/* Name + Email row */}
            <div className="grid sm:grid-cols-2 gap-spacing-lg">
              <div className="space-y-spacing-xs">
                <Label htmlFor="lp-name" className="text-foreground text-sm font-medium">
                  {form?.nameLabel || "Name"}
                </Label>
                <Input
                  type="text"
                  ...
                  className="bg-secondary/70 border-transparent focus-visible:border-border/40 text-foreground"
                />
              </div>
              <div className="space-y-spacing-xs">
                <Label htmlFor="lp-email" className="text-foreground text-sm font-medium">
            ...
            {(form?.selectFields ?? []).map((field: SelectField) => (
              <div key={field.name} className="space-y-spacing-xs">
                {field.label && (
                  <Label htmlFor={`lp-${field.name}`} className="text-foreground text-sm font-medium">
                    {field.label}
                  </Label>
                )}
                <select
                  ...
                  name={field.name}
                  disabled={status === "loading"}
                  className="w-full min-w-0 rounded-md border-0 bg-secondary/70 text-foreground px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring"
            ...
            {/* Message */}
            <div className="space-y-spacing-xs">
              <Label htmlFor="lp-message" className="text-foreground text-sm font-medium">
