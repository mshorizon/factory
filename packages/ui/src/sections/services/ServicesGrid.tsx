...
export function ServicesGrid({
  ...
  return (
    <StaggerContainer
      className={cn(centered ? "grid sm:grid-cols-2 lg:grid-cols-4 gap-spacing-lg" : "grid md:grid-cols-2 lg:grid-cols-3 gap-spacing-lg", className)}
      staggerDelay={0.1}
    >
      {title && (
      ...
      {items.map((item, index) => {
        ...
        const IconComponent = (showIcons || centered) && item.icon ? iconMap[item.icon] : undefined;
        return (
          ...
          <a href={`/services/${item.slug || item.id}`} className="block h-full cursor-pointer" data-field={`items.${index}`}>
            <Card
              className={cn(
                ...
                centered && "items-center text-center !border-0"
              )}
            >
              {IconComponent && (
                centered ? (
