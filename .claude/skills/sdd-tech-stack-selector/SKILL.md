...
description: >
  ...
  "co wybrać do projektu". Use AFTER /sdd-prd, BEFORE /sdd-bootstrapper.
---

# Tech Stack Selector: From PRD to Starter

This skill is the third link in the bootstrap chain (`/sdd-shape → /sdd-prd → sdd-tech-stack-selector → /sdd-bootstrapper`). Its single job: turn a written PRD into a recommended starter and a small machine-readable hand-off `/sdd-bootstrapper` can read to scaffold a project.
...
The starter registry in `references/starter-registry.yaml` is the **single source of truth** for available starters. `/sdd-bootstrapper` reads it; a CI validator (`scripts/validate-starter-registry-sync.mjs`) prevents bootstrapper from referencing a `starter_id` that does not exist here.
...
**Skip when**: `context/foundation/prd.md` is absent — the skill refuses and redirects to `/sdd-shape` + `/sdd-prd`. Skip also when the user is mid-implementation on an existing codebase asking about adding a library or replacing a single dependency — that is `/sdd-frame` territory, not stack selection.

## Relationship to other skills

- `/sdd-shape` — produces `shape-notes.md`, the precursor to PRD. Two upstream of this skill.
- `/sdd-prd` — produces `context/foundation/prd.md`, the canonical input. Always upstream.
- `/sdd-bootstrapper` — downstream consumer. Reads `context/foundation/tech-stack.md` frontmatter and the registry; scaffolds the project.

## Required inputs

1. A PRD file — exists, is readable, conforms to the PRD schema (`/skills/sdd-shape/references/prd-schema.md`). Default location: `context/foundation/prd.md`. The user MAY pass a different path as the argument (see "Initial Response" below). The skill reads the **frontmatter** as priors (`product_type`, `target_scale`, `timeline_budget`, `project`) and may read body sections (`## Functional Requirements`, `## Non-Goals`) for the feature audit and to detect Socratic moments where the PRD's FRs surface a feature the recommended starter does not include.
...
1. **If a path argument was provided** (e.g. `/sdd-tech-stack-selector @context/foundation/prd-v2.md` or `/sdd-tech-stack-selector path/to/prd.md`), strip a leading `@` if present and use the path verbatim as the PRD location for this run.
...
```bash
echo -n "/sdd-shape" | pbcopy 2>/dev/null || echo -n "/sdd-shape" | clip.exe 2>/dev/null || echo -n "/sdd-shape" | xclip -selection clipboard 2>/dev/null || true
```

```powershell
# PowerShell (Windows)
Set-Clipboard "/sdd-shape"
```

Print verbatim (substitute the resolved path; if defaulted, this is `context/foundation/prd.md`):

```
Tech-stack-selector requires a PRD at `<prd-path>`. Run `/sdd-shape` first, then re-invoke.
```
...
- "Stop — fix the PRD first": Exit. Re-run /sdd-prd to fix priors, then re-invoke /sdd-tech-stack-selector.
...
```bash
echo -n "/sdd-bootstrapper" | pbcopy 2>/dev/null || echo -n "/sdd-bootstrapper" | clip.exe 2>/dev/null || echo -n "/sdd-bootstrapper" | xclip -selection clipboard 2>/dev/null || true
```

```powershell
# PowerShell (Windows)
Set-Clipboard "/sdd-bootstrapper"
```
...
═══════════════════════════════════════════════════════════
  ...
  ► Next:      /sdd-bootstrapper  (✓ copied to clipboard)
═══════════════════════════════════════════════════════════
```

STOP. Do not chain into `/sdd-bootstrapper` automatically — the user runs it when ready.
