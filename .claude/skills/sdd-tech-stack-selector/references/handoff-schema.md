...
`context/foundation/tech-stack.md` is the file `/sdd-tech-stack-selector` writes and `/sdd-bootstrapper` reads. This doc is the contract for its shape. Both skills load this file by relative path; renames or restructurings here are load-bearing.
...
Examples: `sdd-astro-starter`, `next`, `t3`, `fastapi`, `django`, `rails`, `spring`, `laravel`, `go`, `rust`, `expo`, `flutter`, `dotnet`.
...
Kebab-case. Drawn from PRD's `project` field unless the user overrode it during the project-name confirmation step. This is the directory name `/sdd-bootstrapper` will scaffold.
