...
1. If PRD body has a `## Forward: tech-stack` block (carried forward from `/sdd-shape`) and that block names a language preference, use it.
...
Ask the user: "Project name for the hand-off (will be the directory name `/sdd-bootstrapper` scaffolds)?" with these options:
