...
expected_recommendation_family: [sdd-astro-starter, t3, next]
...
expected_bootstrapper_confidence_surfaced: first-class  # sdd-astro-starter is first-class
```

Standard path resolves through `recommended_defaults[saas][js] = sdd-astro-starter`. Solo + 4-week timeline + JS family → recommended_default + agent-friendly. (T3 stays as a strong alternative; the recommendation leads with sdd-astro-starter when both compete.)
...
residual_answers:
  language_family: js
q0_expected_path: standard
expected_recommendation_family: [sdd-astro-starter, astro]
expected_socratic_moments:
  - "clarifying scope: static vs dynamic before locking"
expected_bootstrapper_confidence_surfaced: first-class  # sdd-astro-starter
```

Standard path resolves through `recommended_defaults[web][js] = sdd-astro-starter`. For a true static portfolio with no auth/database, Astro alone may be the better pick — the skill should ask one clarifying question.
...
residual_answers:
  ...
  tech_preferences:
    avoid: [php]
q0_expected_path: standard
expected_recommendation_family: [sdd-astro-starter, t3]
expected_socratic_moments:
  - "if user later flips and asks about Laravel, Socratic challenge fires comparing against quality-bar"
expected_bootstrapper_confidence_surfaced: first-class
```

Standard path; recommended_defaults[saas][js] = sdd-astro-starter. The avoid: [php] entry means Laravel is silently dropped from any candidate list (Step A filter 4). If the user later names Laravel anyway, Step B's Socratic challenge fires (Laravel passes the gates, but the user's avoid-list takes precedence — the challenge surfaces the conflict, not a quality failure).
...
residual_answers:
  language_family: js
  user_explicit_request: "I want sdd-astro-starter specifically"
q0_expected_path: standard  # recommended default for (web, js)
expected_recommendation_family: [sdd-astro-starter]
expected_socratic_moments: []
expected_bootstrapper_confidence_surfaced: first-class  # registry says first-class until promoted
```

Verifies Step E surfaces the `bootstrapper_confidence: first-class` value verbatim in conversation: "Bootstrapper has this stack registered with a valid CLI but hasn't been battle-tested. Expect mostly-smooth scaffolding with occasional manual steps." The user proceeds knowingly. Note the `sdd-astro-starter` card stays at `first-class` until verified end-to-end through bootstrapper, at which point promote to `verified`.
