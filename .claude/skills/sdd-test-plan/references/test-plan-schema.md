...
This is the canonical structure for the test plan produced by `/sdd-test-plan`. The file is **both a phased rollout strategy and a growing cookbook**:
...
- **Cookbook** (§6) starts as placeholders and **fills in incrementally** as rollout phases ship. Each plan's final sub-phase (per the `/sdd-plan` constraint baked in by the orchestrator) updates the cookbook with patterns the rollout phase just delivered.
...
>
> Refresh: re-run `/sdd-test-plan --refresh` when stale (see §8).
>
...
3. **Risks are scenarios, not code locations.** This plan documents *what
   ...
   produced by `/sdd-research` during each rollout phase. If the plan and
...
Short by design. Tone is imperative — these are the rules every downstream phase obeys. Principle #3 is the load-bearing distinction between this plan (QA spec) and `/sdd-research` (knowledge extraction).
...
`/sdd-research` during each rollout phase. If the plan tries to cite them
...
is the durable handoff from risk analysis to `/sdd-research` and
`/sdd-plan`.

```markdown
### Risk Response Guidance

| Risk | What would prove protection | Must challenge | Context `/sdd-research` must ground | Likely cheapest layer | Anti-pattern to avoid |
|------|-----------------------------|----------------|--------------------------------------|-----------------------|-----------------------|
...
via `/sdd-new`. Status moves left-to-right through the values below; the
...
those belong in per-phase `/sdd-research`.
```
...
**This is the section that fills in over time.** The orchestrator writes placeholders during Phase 4; each rollout phase's plan updates the relevant sub-section in its final sub-phase (per the `/sdd-plan` constraint).
...
(Optional. After each phase lands, /sdd-implement appends a 2-3 line note
...
Refresh (`/sdd-test-plan --refresh`) when:
...
- **No motivational phrasing** ("write better tests", "be a SDD engineer").
- **No emojis.**
...
## How `/sdd-test-plan` maintains this file
...
- **After each rollout phase completes** (via `/sdd-implement`'s final sub-phase, baked into the plan by the orchestrator's planning prompt): the relevant §6 sub-section is filled in; §8 dates may bump if tools were verified.
