...
```
No stack-assessment.md found. Run /sdd-stack-assess for quality-gate analysis.
```
...
When the health-check runs standalone (outside the course chain), all findings go into a single ranked list without the A/B split. Enrich lesson references with titles and links per the mapping in SKILL.md.
