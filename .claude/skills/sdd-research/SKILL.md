...
6.  **Resolve change folder and gather metadata for the research document:**
    ...
    - Determine the change-id:
      - If invoked as `/sdd-research <change-id>` and `context/changes/<change-id>/` exists, use it.
      - Otherwise derive a kebab-case change-id from the topic and create the folder + `change.md` (mirroring `/sdd-new` semantics) before writing.
