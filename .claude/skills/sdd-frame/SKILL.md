...
description: >
  ...
  the assumption", "rethink", "before I plan". Use BEFORE /sdd-plan, not in
  place of it.
---
...
The shape this skill addresses is general: the user describes something (an observation, a perceived problem, a scope they want to take on) and proposes a response (a cause, an approach, a plan structure) in the same breath. The two get treated as one fact. A perfect /sdd-plan then ships a perfect solution — and the actual problem remains, because the framing was wrong; the plan was right; the user lost the day.

This skill is the framing step. /sdd-plan answers _how to build it_. /sdd-frame answers _what's actually the right thing to plan_.

## When to use, when to skip

**Use when**: input has bug-shape ("X is broken, let's build Y"), scope-shape ("we should split this into two plans", "is this even the right scope"), design-shape ("which approach do we even want"), or assumption-shape ("we're assuming X — is that true?"). Use also when stakes are high, when the system is unfamiliar to the user, or when /sdd-plan is about to start on a task that smells like a stated cause rather than a verified one.
...
- `/sdd-research` — broad codebase exploration. Frame can consume a research doc as input but does not replace it.
- `/sdd-plan` — accepts frame output as input. The Frame Brief IS a valid first argument to /sdd-plan.
- `/sdd-plan-review` — validates an existing plan. Frame validates the _premise_ before a plan exists.

The Frame Brief is useful standalone (as a discussion artifact, or to scope a quick fix) — it does not require /sdd-plan to follow.

## Initial Response

When this skill is invoked:

1. **If a file path or change-id was provided** (e.g. `/sdd-frame @context/changes/foo/research.md` or `/sdd-frame foo`), resolve it: a `<change-id>` resolves to `context/changes/<change-id>/research.md` (read it if present). Read the file FULLY and proceed to Step 1.
...
Tip: pass research directly — `/sdd-frame @context/changes/<change-id>/research.md` (or just `<change-id>`)
```
...
Ask the user: **The questions and options here are fundamentally different from /sdd-plan's**: in /sdd-plan, options are _solution choices_; here, options are _hypothesis disambiguators_. The user's answer narrows the hypothesis space.
...
- If invoked as `/sdd-frame <change-id>` and `context/changes/<change-id>/` exists, write into it.
- Otherwise derive a kebab-case `<change-id>` from the observation and create the folder + `change.md` (mirroring `/sdd-new` semantics) before writing.
...
> Framing step before /sdd-plan. This document captures what is _actually_
...
[Pick one. If LOW, list the specific verification step needed before /sdd-plan.]

## What Changes for /sdd-plan
...
- question: "Frame done. How would you like to proceed?"
  header: "Next step"
  options:
  - label: "Hand off to /sdd-plan"
    description: "Pass this brief to /sdd-plan and start implementation planning."
...
If the user picks "Hand off to /sdd-plan", copy the command to clipboard:

```bash
echo -n "/sdd-plan <change-id>" | pbcopy 2>/dev/null || echo -n "/sdd-plan <change-id>" | clip.exe 2>/dev/null || echo -n "/sdd-plan <change-id>" | xclip -selection clipboard 2>/dev/null || true
```

```powershell
# PowerShell (Windows)
Set-Clipboard "/sdd-plan <change-id>"
```

And print: `→ /sdd-plan <change-id> (✓ copied)`
...
2. **Observation and stated cause stay separate.** Through every step. The Frame Brief preserves the original framing verbatim — even when reframed — because future readers (and /sdd-plan-review) need to see what was assumed vs what was discovered.

3. **No solution design.** This skill never picks an implementation approach. It does not propose phases, file changes, or technical decisions. It produces ONE artifact: the reframed (or confirmed) problem statement. /sdd-plan owns the solution.

4. **Narrowing questions ≠ solution questions.** /sdd-plan asks "which approach?". /sdd-frame asks "where in the dimension map does the actual problem live?". This rule binds both Step 1.5 (pre-dispatch scope/observation narrowing) and Step 4 (post-dispatch hypothesis narrowing). Options describe observations or design positions, not choices about how to address them. If you find yourself drafting a question whose answer changes the _direction_, you've crossed into /sdd-plan territory — stop.
...
- The Frame Brief is the only artifact. Keep it short, scannable, and actionable for /sdd-plan.
