...
description: >
  ...
  greenfield or brownfield — into shape-notes.md, the input to /sdd-prd.
  ...
  Use BEFORE /sdd-prd, not in place of it.
---

# Shape: Facilitate Discovery (Greenfield & Brownfield) Before /sdd-prd

This skill is the head of the bootstrap chain. For greenfield: `/sdd-shape → /sdd-prd → sdd-tech-stack-selector → bootstrapper`. For brownfield: `/sdd-shape → /sdd-prd → sdd-stack-assess → sdd-health-check`. Its single job: walk a user from "I have an idea" (greenfield) or "I want to change this system" (brownfield) to a structured `shape-notes.md` — written into a per-project subfolder `context/<app>/foundation/<project-slug>/shape-notes.md` (see the schema's "Output paths" section) so multiple apps and projects coexist — that `/sdd-prd` can turn into a PRD that conforms to the locked schema.
...
The locked schema both this skill and `/sdd-prd` conform to lives at `references/prd-schema.md` (relative to this SKILL.md). Read it before producing any artifact and re-check against it at every checkpoint write.

## Repo profile — product-feature delivery mode

This skill defaults to **`delivery_mode: product-feature`**: features are specified by product/design and built **end to end** — there is no MVP scoping and no personal-timeline budget unless the user EXPLICITLY asks for MVP framing (says "MVP", "smallest slice", "prototype", or similar — that overrides the default back to `mvp` for the session).

Concretely, in product-feature mode:

- **Step 3** runs as **"End-to-end flow & success criteria"**, not "MVP discipline": capture the full feature flow as the Primary success criterion; SKIP the three-week scope-cost surface, the after-hours framing, and the `mvp_weeks` / `delivery_weeks` capture entirely. Scope-down advice remains available as an *offer* when the user themselves hesitates about size — never as a gate.
- **Step 6 timing**: ask only whether a hard deadline / commitment date exists; do NOT ask "after-hours or day job". Write `timeline_budget: null` (or `{ hard_deadline: <date> }` when one exists).
- **Step 7 check #4** (Timeline-cost acknowledged) is **`n/a (product-feature)`** — print it as such; it never counts as missing.
- Write `delivery_mode: product-feature` into shape-notes.md frontmatter (alongside `context_type`) immediately after Step 0.7, so `/sdd-prd` carries it into the PRD.

Everything else (facilitation pattern, FRs, Socrates round, business logic, non-goals, quality gate) runs unchanged — the mode only removes the MVP/timeline pressure, not the discovery rigor.

## When to use, when to skip

**Use when**: the user describes a new project idea (greenfield), a meaningful change to an existing system — new module, significant feature, architectural improvement (brownfield), or a product they want to rebuild from first principles. Use also when an existing `context/<app>/foundation/<project-slug>/shape-notes.md` is incomplete and needs resuming. The skill auto-detects context type from project markers in cwd and adapts.

**Skip when**: the project already has a PRD or an ADR set (use `/sdd-frame` or `/sdd-plan` instead), or the user is reasoning about a single bug / refactor / small feature within an existing codebase that doesn't warrant a full PRD (use `/sdd-frame`). For brownfield projects where the user wants to shape a meaningful change, this skill IS the right starting point.

## Relationship to other skills

- `/sdd-init` — scaffolds the `/context` skeleton plus a root README documenting the workspace-first layout. Per-app `foundation/`, `changes/`, and `archive/` folders are created lazily by the entry skills. `/sdd-shape` requires `context/` to exist; if absent, it delegates to `/sdd-init` via a tool call (Step 0 below).
- `/sdd-prd` — consumes `shape-notes.md`. The handoff is the `## Step 8` clipboard write.
- `/sdd-frame` — for _reframing_ small-scope problems within existing systems where a full PRD is overkill. `/sdd-shape` is for larger brownfield changes (new modules, significant features) that need structured discovery and a PRD.
- `/sdd-stack-assess` — downstream of `/sdd-prd` for brownfield projects. Evaluates existing stack against quality gates.
- `/sdd-health-check` — downstream of `/sdd-stack-assess` for brownfield. Audits existing project health.
- `/sdd-plan` — downstream of `/sdd-prd`, never invoked from here directly.

## Initial Response

When this skill is invoked:

1. **If a freeform idea was provided as the argument** (e.g. `/sdd-shape a recipe app that suggests meals from what's in your fridge`), capture it verbatim as the **seed idea**. Do not rephrase. Proceed to Step 0.
2. **If a file path was provided** (e.g. `/sdd-shape @notes/idea.md`), read it FULLY and use its contents as the seed. Proceed to Step 0.
3. **If nothing was provided**, respond with:

```
I'll help you shape an idea into structured notes that /sdd-prd can turn into
...
Tip: pass the idea inline — `/sdd-shape a recipe app that uses fridge contents`
     or for brownfield — `/sdd-shape add a recommendation engine to my recipe app`
```
...
### Step 0: Check SDDWorkflow precondition

Check the SDDWorkflow scaffold:

```bash
test -d context
```

If it exists, proceed to Step 0.5. (Per-app `foundation/` folders are created lazily — their absence does not mean the project is uninitialized; only the top-level `context/` matters here.)

If missing, the project has not been initialized for SDDWorkflow. Ask the user:
Ask the user: "This directory isn't initialized for SDDWorkflow (context/ is missing). Run /sdd-init now?" with options:

- "Yes — run /sdd-init (Recommended)" (description: "Scaffolds the /context skeleton (changes/, archive/, foundation/) with READMEs, then continues shaping.")
- "No — stop here" (description: "Exit without changes. You'll need to initialize before shape can run.")

On "Yes": invoke `/sdd-init` via a tool call (NOT via Bash). When `/sdd-init` returns, re-check the precondition; if it now passes, continue to Step 0.5. On "No": print "Stopping. Run `/sdd-init` when ready, then re-invoke `/sdd-shape`." and STOP.

Do not duplicate `/sdd-init`'s scaffold logic. A tool call is the correct delegation path.

### Step 0.5: Resume detection

Scan for prior sessions across every app's project subfolders (the leading `*` is the app segment — do NOT collapse it to a single app):

```bash
ls context/*/foundation/*/shape-notes.md 2>/dev/null
```

- **No matches** → fresh session; proceed to Step 0.6.
- **One match** → read it FULLY and offer resume (below).
- **Multiple matches** → list each candidate (project name + current phase) and ask the user which to resume, OR to start a new project. If they pick "new project", proceed to Step 0.6; otherwise continue below with the chosen file.

For the chosen prior session, read the file FULLY. Parse the frontmatter `checkpoint:` block per the schema reference (`references/prd-schema.md`, "shape-notes.md checkpoint format" section). Extract: `current_phase`, `phases_completed`, `frs_drafted`, `quality_check_status`. Set the session's **`ACTIVE_SHAPE_PATH`** to that file's path — its parent subfolder is the project folder for all subsequent writes; do NOT create a new one.

Summarize what you found:

```
Found a prior shape session at <ACTIVE_SHAPE_PATH>:
...
- "Restart from scratch" (description: "Archive the existing shape-notes.md and start a new session.")
- "Cancel" (description: "Exit without changes.")

On "Resume": jump directly to the next unfinished phase (Step `current_phase` + (1 if current is in `phases_completed` else 0)). Do NOT re-run completed phases — only summarize each one back to the user in 1–2 sentences ("Phase 1 captured: <one-line problem>; Phase 2 captured: <one-line persona>; …") so they have context for what was already decided. **Skip Step 0.6** — the project subfolder already exists (it's `ACTIVE_SHAPE_PATH`'s parent).

On "Restart": the app is already known from the existing file's path (`context/<app>/foundation/…`). Move the file to `context/<app>/foundation/archive/shape-notes-<project-slug>-<YYYY-MM-DD-HHMM>.md` (create the archive directory if absent), then proceed to Step 0.6 with a fresh session (the app is already resolved — skip its re-resolution).

On "Cancel": STOP without changes.

### Step 0.6: App resolution & project identity & subfolder

**(Fresh sessions only — skipped when resuming in Step 0.5.)** Establish the application and the project subfolder before any write happens, so the app + project name are known up front and outputs never clobber another app's or project's.

This is the primary ENTRY skill — it creates the project foundation folder, so it OWNS app resolution. SDD artifacts are partitioned **by workspace** when the repo uses a monorepo/workspace layout: `<app>` is the resolved workspace directory's name (e.g. `assets`, `campaigns`, `crm`) and is the first path segment of every artifact (`context/<app>/…`). Resolve it once here, then persist it; downstream skills read the app from the path they are handed and never re-resolve it.

**App resolution ("ask once, store it")**:

1. **Determine the workspace parent** — probe in order, first match wins, and use it as `<workspace-parent>` below:
   - `packages/apps` — if `packages/apps/` exists
   - `apps` — if `apps/` exists
   - the directory holding the `workspaces` (root `package.json`) or `packages:` (`pnpm-workspace.yaml`) globs, if either declares one
   - **none of the above → flat repo**: skip resolution and use the FALLBACK below.

2. **Auto-detect from cwd** — if this skill was invoked inside a workspace dir, propose that `<app>`. Detect via (substitute the resolved `<workspace-parent>`):

   ```bash
   pwd | sed -nE 's|.*/<workspace-parent>/([^/]+).*|\1|p'
   ```

3. **Otherwise, list the workspaces and ask** — run `ls -1 <workspace-parent>/` and ask the user to pick one (use this skill's standard interactive-question mechanism, the same one every other step uses).

   ```bash
   ls -1 <workspace-parent>/
   ```

4. **Confirm** the chosen `<app>` with the user (when auto-detected, confirm the proposal; when listed, the pick is the confirmation).

**FALLBACK (flat repos — no workspace layout / single-package / greenfield)**: if step 1 finds no workspace parent, skip the `<app>` segment entirely — use the flat layout `context/{foundation,changes,archive}/…`, set `ACTIVE_SHAPE_PATH` = `context/foundation/<project-slug>/shape-notes.md`, and OMIT the `app:` frontmatter field. Everything else in this step is unchanged; `<app>` simply drops out of the paths. This fallback applies wherever `<app>` appears below.

Then propose a short proper-noun **project name** derived from the seed idea, and derive its **slug** per the schema's "Output paths" rule (lowercase; each run of non-alphanumeric characters → a single hyphen; trim leading/trailing hyphens). E.g. a seed about personalising Brickworks schema fields → name "Brickworks Personalisations" → slug `brickworks-personalisations`.

Confirm with the user:
Ask the user: "I'll save this project's shaping + PRD under `context/<app>/foundation/<proposed-slug>/`. Use this name?" with options:

- "[Proposed name] — yes (Recommended)" (description: "Create context/<app>/foundation/<proposed-slug>/ and shape into it.")
- "Rename" (description: "You give the project name; I'll re-derive the slug.")

On confirm (or after the user renames), derive the final slug, create the folder lazily, and set the active path:

```bash
mkdir -p "context/<app>/foundation/<project-slug>"
```

Set **`ACTIVE_SHAPE_PATH` = `context/<app>/foundation/<project-slug>/shape-notes.md`**. Every subsequent phase write targets this path (when later steps say "write to `shape-notes.md`", they mean `ACTIVE_SHAPE_PATH`). Write the initial frontmatter scaffold here now, with `app: <app>` and `project: '<name>'` already populated (`project` is no longer `null` — the name is known) plus the `checkpoint:` block; `context_type` and `delivery_mode` are filled in by Step 0.7. _(Fallback: repos with no workspace layout omit the `app:` line.)_

If `context/<app>/foundation/<project-slug>/shape-notes.md` already exists, do not overwrite — treat it as a resume candidate and return to Step 0.5's resume offer for that file.
...
Write the confirmed `context_type` into the shape-notes.md frontmatter (alongside `checkpoint:`) immediately. This value is load-bearing for `/sdd-prd`'s auto-routing.
...
- NEVER pre-commit to a stack (framework, database, hosting platform, language family). The PRD captures product-level priors only — `product_type`, `target_scale`, `timeline_budget`. Stack-shaped concerns are gathered downstream of `/sdd-prd`.
- NEVER use certification language in shipped output. The mechanics here are universal indicators of a well-scoped project. The user-facing artifact reads as a generic shaping skill.
...
> **Product-feature mode** (the default in this repository — see *Repo profile* above): skip every timeline question and scope-cost surface in this step. Open with the flow question reframed to the FULL feature ("Walk me through the end-to-end flow of the finished feature, click by click" / for brownfield: "…how a user's experience changes once the whole change ships"), echo it back as a numbered sequence, ask the brownfield blast-radius question if applicable, then go straight to **Both modes** below to capture Primary / Secondary / Guardrails. Do not set `mvp_weeks` / `delivery_weeks`; `timeline_budget` stays `null` until Step 6's hard-deadline question.
...
Set `timeline_budget.mvp_weeks` (greenfield) or `timeline_budget.delivery_weeks` (brownfield) in the frontmatter scaffold to the user's number — 1 if scoped down, the acknowledged estimate otherwise. *(Product-feature mode: skip — `timeline_budget` stays `null`.)*
...
PRD frontmatter is product-level only. Stack-shaped concerns — team composition, language preferences, technology avoid-lists, deployment mode/region/budget, CI/CD pipeline shape — and architectural commitments — implementation decisions, testing strategy, deployment plan — are NOT part of the PRD. They are gathered downstream of `/sdd-prd`, after the product shape is locked. Asking them now invites the user to over-commit before stack selection has happened, and the answers usually need to be revisited once the stack is picked.

> **Product-feature mode**: the timing questions in both modes below reduce to the hard-deadline question only ("Is there a release/commitment date for this feature?"). Do NOT ask "after-hours or day job". Write `timeline_budget: null`, or `{ hard_deadline: <YYYY-MM-DD> }` when a date exists.
...
═══════════════════════════════════════════════════════════
  ...
  Timeline-cost ack:        [present | missing — describe | n/a (product-feature)]
  Non-Goals:                [...]
...
- "Accept and finish" (description: "Proceed despite the gaps. They will be recorded as warnings in the checkpoint and surfaced in /sdd-prd's Open Questions.")
...
- Re-validate against the schema reference one more time: for greenfield, the body should anticipate the 10 PRD sections in the order the schema requires; for brownfield, the 11 brownfield PRD sections. The frontmatter should be the full `checkpoint:` block plus `context_type`, plus `app: <app>` in workspace repos (omitted in repos with no workspace layout). Any forward-looking content captured in Step 6 stays in its `## Forward: ...` block — NOT folded into PRD-schema sections.

Then copy the next-step command to clipboard and announce:

```bash
echo -n "/sdd-prd" | pbcopy 2>/dev/null || echo -n "/sdd-prd" | clip.exe 2>/dev/null || echo -n "/sdd-prd" | xclip -selection clipboard 2>/dev/null || true
```

```powershell
# PowerShell (Windows)
Set-Clipboard "/sdd-prd"
```
...
═══════════════════════════════════════════════════════════
  ...
  ► Notes:  context/<app>/foundation/<project-slug>/shape-notes.md
  ► Next:   /sdd-prd  (✓ copied to clipboard — point it at the path above)

  After /sdd-prd, the next chain step will pick up:
...
STOP. Do not chain into `/sdd-prd` automatically — the user runs it when ready.
...
5. **Soft gate, not hard gate.** The closing cross-check WARNS but allows the user to override every gap. Override paths are recorded in the checkpoint as `quality_check_status: warned` and surfaced in `/sdd-prd`'s `## Open Questions`. Refusing to finish is not in scope.

6. **Mode-aware behavior.** The skill auto-detects context type (greenfield vs brownfield) from project markers in cwd and adapts all six discovery phases accordingly. For brownfield, the discovery loop shifts from "what are you building from scratch?" to "what exists, what's changing, what must be preserved?". If the user invokes this skill for a small-scope problem within an existing codebase (single bug, quick refactor), suggest `/sdd-frame` instead — `/sdd-shape` is for changes that warrant a full PRD.

7. **Universal language only.** No certification references in any user-facing output or any artifact written to disk. The mechanics here are universal indicators of a well-scoped project; the persona context that motivated them lives in the change folder, not in the shipped skill.
...
- This is a **shaping** skill. Output is `shape-notes.md`, not `prd.md`. `/sdd-prd` is the document generator.
