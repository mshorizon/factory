...
This doc is the single source of truth for the shape of the `prd.md` produced by `/sdd-prd`, and for the `checkpoint:` block written into the `shape-notes.md` by `/sdd-shape`. Both skills load this file by relative path (`skills/sdd-shape/references/prd-schema.md`) and conform to it.

# Output paths (workspace-first convention)

Artifacts are partitioned by **application** first when the repository hosts more than one app (a monorepo). Each app owns its own `foundation/`, `changes/`, and `archive/` trees, and all shape/PRD outputs for one project/feature live in a project subfolder under that app's `foundation/`:

```
context/<app>/foundation/<project-slug>/
├── shape-notes.md      # written by /sdd-shape
├── prd.md              # written by /sdd-prd
├── prd-vN.md           # versioned PRD saves (collision resolution)
└── roadmap.md          # written by /sdd-roadmap (sibling of prd.md)
```

**App** — the resolved workspace directory's name (e.g. `assets`, `campaigns`, `crm`). `/sdd-shape` resolves it once — auto-detecting from cwd when invoked inside a workspace dir, otherwise asking the user — then persists it as the `app:` frontmatter field and as the leading path segment. Downstream skills (`/sdd-prd`, `/sdd-roadmap`, `/sdd-plan`, …) read the app from the artifact path they are handed and never re-resolve it. **Repos with no workspace layout** (single-package or greenfield) omit the `<app>` segment entirely and use the flat layout `context/{foundation,changes,archive}/…`; in that case `app:` is omitted from frontmatter.

**Project slug** — derived deterministically from the `project` name: lowercase it, replace each run of non-alphanumeric characters with a single hyphen, and trim leading/trailing hyphens. Example: `"Brickworks Personalisations"` → `brickworks-personalisations`. The slug is fixed once the subfolder is created (Step 0.6 of `/sdd-shape`); refining the human-readable `project` name later does not rename the folder.

Per-change artifacts live under the same app: `context/<app>/changes/<change-id>/` (`change.md`, `plan.md`, research, frame, reviews). Completed changes archive to `context/<app>/archive/<change-id>/`.

`/sdd-shape` creates `context/<app>/foundation/<project-slug>/` up front and writes `shape-notes.md` into it. `/sdd-prd` writes `prd.md` as a **sibling of its input** (the same subfolder the `shape-notes.md` lives in). Downstream skills that consume `prd.md` receive the subfolder path explicitly rather than assuming a fixed top-level location.
...
app: <string> # workspace repos only: the resolved workspace dir name (e.g. assets). First path segment context/<app>/… . Omit in repos with no workspace layout.
...
delivery_mode: <enum> # mvp | product-feature — defaults to mvp when absent
...
timeline_budget:
  <object|null> # greenfield (mvp mode): { mvp_weeks: <int>, hard_deadline: <YYYY-MM-DD | null>, after_hours_only: <bool> }
  # brownfield (mvp mode): { delivery_weeks: <int>, hard_deadline: <YYYY-MM-DD | null>, after_hours_only: <bool> }
  # product-feature mode: null, or { hard_deadline: <YYYY-MM-DD> } when an external commitment date exists
---
```

Field semantics:

- **`project`** — short proper-noun name. The skill never invents this; if the user did not name the project, `/sdd-shape` asks before `/sdd-prd` runs.
- **`app`** — _(workspace repos only)_ the workspace this project belongs to, equal to its resolved directory name (e.g. `assets`, `campaigns`). Load-bearing: it is the first path segment of every artifact (`context/<app>/…`). `/sdd-shape` resolves it once (cwd auto-detect → confirm, or asks) and writes it here; `/sdd-prd` and all downstream skills copy it through and derive paths from it. Never inferred from the project name. Omitted in repos with no workspace layout, which use the flat `context/{foundation,changes,archive}/` layout.
- **`version`** — starts at `1`. Versioned saves (`prd-v2.md`, `prd-v3.md`) increment this so downstream tooling can sort by recency.
- **`context_type`** — `greenfield` for new projects built from scratch; `brownfield` for changes to existing systems. Determines which section list (10 greenfield vs 11 brownfield) the PRD uses. Set by `/sdd-shape`'s auto-detection; `/sdd-prd` reads it from `shape-notes.md` frontmatter.
- **`status`** — `draft` on first write. `/sdd-prd` does not promote past `draft`; downstream skills (review, sign-off) flip it.
...
- **`delivery_mode`** — `mvp` (default) or `product-feature`. `product-feature` marks a repo/org where features are specified by product/design and shipped **end to end** — not scoped down to an MVP slice — typically a product company's main codebase. In this mode `timeline_budget` is `null` (or carries only `hard_deadline` when an external commitment date exists), the MVP/timeline-discipline questions are skipped, and Success Criteria describe the full feature. Set by `/sdd-shape` (repo profile or explicit user statement); `/sdd-prd` copies it through. A user explicitly asking for an MVP overrides the repo default back to `mvp` for that session.
- **`timeline_budget.mvp_weeks`** (greenfield) / **`timeline_budget.delivery_weeks`** (brownfield) — integer week count for the first shippable version / change. Applies in `mvp` mode only. Three weeks of after-hours work is a good default target — short enough to maintain momentum, long enough to build something real. Longer timelines are valid when the team has acknowledged the sustained-effort cost up front. In `product-feature` mode these keys are absent (`timeline_budget: null`).

Stack-shaped concerns — team composition, language preferences, technology avoid-lists, deployment mode, region, budget tier, CI/CD shape — are intentionally absent from this frontmatter. They belong to the tech-stack-selection step that runs after `/sdd-prd` and are gathered there. If `/sdd-shape` captures forward-looking notes about them, those notes live in `shape-notes.md` body, not in PRD frontmatter or sections.
...
If `/sdd-shape` ran a Socratic round, each FR may carry a `> Socratic:` blockquote underneath capturing the strongest counter-argument and the user's resolution.
...
If `/sdd-shape` flagged the empty-CRUD anti-pattern and the user accepted the warning, this section still exists with the user's chosen domain rule. If they overrode without choosing one, the section reads `# TODO: domain rule — see Open Questions` and `## Open Questions` carries an entry naming the gap.
...
Numbered list. Each entry names what's unknown, who needs to resolve it, and the latest acceptable resolution date if any. The `/sdd-prd` skill ALWAYS routes captured uncertainty here verbatim — it never invents domain decisions to fill gaps.
...
`/sdd-shape` writes `context/<app>/foundation/<project-slug>/shape-notes.md` (see "Output paths" above) with a frontmatter `checkpoint:` block that drives resume behavior. The block's shape is fixed; the body of `shape-notes.md` (the captured discussion, draft FRs, draft user stories) is freeform and may evolve.

```yaml
---
project: <string> # mirrors PRD frontmatter; set in Step 0.6 (names the subfolder), refinable later
context_type: <enum> # greenfield | brownfield — set during Step 0.7, load-bearing for /sdd-prd routing
delivery_mode: <enum> # mvp | product-feature — mirrors PRD frontmatter; set from repo profile or user statement
created: <YYYY-MM-DD>
...
2. Grep both skill bodies (`skills/sdd-shape/SKILL.md`, `skills/sdd-prd/SKILL.md`) for the old name; update.
