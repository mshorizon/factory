---
name: sdd-e2e
description: Drive an approved plan's browser-level (E2E) phases against the running app, one risk at a time — plan → generate → review → verify. The E2E sibling of /sdd-implement and /sdd-tdd, sharing the same plan and Progress. Only drives risks that genuinely need a browser and whose feature is already built; redirects the rest to /sdd-tdd or /sdd-implement. Use when the user says "e2e", "write/generate a Playwright test", "browser test this risk", or "drive the plan's E2E phases".
---

# SDD E2E — Risk-Driven E2E Plan Execution
...
This skill is the **E2E sibling of `/sdd-implement` and `/sdd-tdd`**. It reads the same plan, mutates the same canonical `## Progress` section, and uses the same phase-end commit ritual and clipboard handoffs. The difference is the inner loop: instead of writing production code (`/sdd-implement`) or a failing unit test first (`/sdd-tdd`), you generate and harden a browser-level test against the **running app**. Because the three skills share `## Progress`, you can interleave them freely — build a feature with `/sdd-implement`, unit-test the next phase with `/sdd-tdd`, then come back here to add the E2E layer for a cross-boundary risk, and state is never lost.

Besides this plan-driven path, `/sdd-e2e` also runs **standalone** against a single risk: `/sdd-e2e <risk-id>` (or no argument) reads `context/foundation/test-plan.md`, picks the top browser-level risk, and produces **one** reviewed, break-verified test, then stops — no change folder, no `## Progress`, no commit ritual. Use it for a quick one-off outside a tracked change. Everything below about **Setup**, **Phase completion**, and **State tracking** applies to the plan-driven path only; a standalone run goes straight to the gate, loops PLAN→GENERATE→REVIEW→VERIFY once, and reports the test.
...
- **The feature under test already exists.** E2E runs against a real, running app — so unlike `/sdd-tdd`, the implementation must be **present**, not absent. If the phase's feature isn't built yet, there is nothing for the browser to drive; stop and redirect to `/sdd-implement` (or `/sdd-tdd`) to build it first, then come back.
...
After all phases →  Completion summary + optional /sdd-impl-review
```
...
1. **Resolve the plan**:
   - `/sdd-e2e <change-id> [phase N]` → `context/changes/<change-id>/plan.md`.
   - `@context/changes/<change-id>/plan.md` or a full path → accept as-is.
   - **Refuse if the resolved path starts with `context/archive/`** — print "This change is archived. Open a new change with `/sdd-new` instead." and STOP.
...
1. A change-id (e.g., `/sdd-e2e save-session phase 6`), or
...
This skill assumes Playwright is already installed; it won't set it up. Options:
  • Install and configure Playwright first (npm init playwright@latest), then re-run /sdd-e2e.
  • Use /sdd-tdd or /sdd-implement for non-browser coverage.
```
...
A risk needs E2E when it **crosses several system boundaries** (auth, routing, API, DB) or **exists only in the rendered UI**. If an isolated function, endpoint contract, or integration test could prove the risk, E2E is the wrong (slow, brittle) tool — drive it with `/sdd-tdd` or `/sdd-implement` instead.

| E2E-worthy — drive it here | Not E2E-worthy — redirect to /sdd-tdd or /sdd-implement |
|---|---|
...
### Feature-presence check (the inverse of /sdd-tdd)
...
Copy `/sdd-implement <change-id> phase [N]` to the clipboard, display it with `(✓ copied)`, and STOP.
...
- If it's **mixed or ambiguous** (e.g., a phase that's partly an endpoint contract, partly a rendered-UI flow), Ask the user: "Phase [N] mixes an isolated-function risk and a browser-level flow. How should I drive it?"
    - Option 1: "E2E the browser-level part (Recommended)" (I'll plan→generate→review→verify the cross-boundary flow and redirect the isolated-function part to /sdd-tdd.)
    - Option 2: "Redirect whole phase to /sdd-tdd" (Hand the entire phase off — copy the resume command to the clipboard.)
...
State *why* the phase isn't a browser-level fit (one or two sentences, grounded in the table above), then Ask the user: "Phase [N] isn't a good E2E fit. How do you want to handle it?"
  - Option 1: "Hand off to /sdd-tdd (Recommended)" (Copy `/sdd-tdd <change-id> phase N` to the clipboard. Clear context, run it, then resume E2E on the next phase.)
  - Option 2: "Hand off to /sdd-implement" (Copy `/sdd-implement <change-id> phase N` to the clipboard if test-first doesn't fit either.)
...
→ /sdd-tdd <change-id> phase [N] (✓ copied)

Clear context (`/clear`), run that, then come back with:
→ /sdd-e2e <change-id> phase [N+1]
```
...
When all `#### Automated` rows in `### Phase N:` are `[x]`, run the phase-end ritual (this mirrors `/sdd-implement` and `/sdd-tdd` — one Conventional-Commits commit per phase, then write its short SHA back into the rows that flipped).
...
8. **Capture and write back the SHA.** `git rev-parse --short HEAD` → `SHA`. For every Progress row flipped this phase, modify `- [x] N.M <title>` → `- [x] N.M <title> — <SHA>` (skip rows that already carry a SHA; if `SHA=""`, skip — `/sdd-archive` surfaces SHA-less rows as informational warnings).
...
Ask the user: "Phase [N] complete (E2E). How to proceed?"
  ...
  - Option 3: "Review this phase first" (Run /sdd-impl-review to verify the implementation against the plan before continuing.)

**Continue:** read the next phase, set its task `in_progress`, run the E2E gate, proceed. No need to re-read the whole plan.

**Review:** run `/sdd-impl-review @<path-to-plan> phase [N]`, then re-present the continue/clear decision (without the review option).

**Clear:** copy `/sdd-e2e <change-id> phase [N+1]` to the clipboard (per the clipboard convention) and display it as `→ /sdd-e2e <change-id> phase [N+1] (✓ copied)`.
...
**The `## Progress` section in `plan.md` is the single source of truth** — no state file, no comment markers. This skill mutates Progress exactly like `/sdd-implement` and `/sdd-tdd`: flip `[ ]` → `[x]` per step as it lands; append the closing commit's SHA to every row that flipped, in one shot at phase end. Mid-phase, completed rows sit `[x]` without a SHA — a valid intermediate state. Because all three skills write the same section identically, a change can be driven by any of them, in any order.
...
2. **Update `change.md`**: `status: implemented`, `updated: <today>`. (Do NOT set `archived_at` — that's `/sdd-archive`.)
...
Summary:
- Phases completed: [N]  ([k] E2E'd, [j] redirected to /sdd-tdd or /sdd-implement)
- E2E tests added: [count] across [files], each tied to a test-plan.md risk
- Levers in place: seed.spec.ts + E2E rules
```

   Then Ask the user: run `/sdd-impl-review <change-id>` (full-plan review) or skip.
