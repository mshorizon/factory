...
- Task + research doc (`context/<app>/changes/<change-id>/research.md`) → fewer questions; I won't redo what research covered
- Task + frame brief (`context/<app>/changes/<change-id>/frame.md`) → far fewer questions; the problem framing is already settled
- Task + frame + research → minimum questions; I focus only on solution-design decisions that need your input

Tip: invoke directly with a change-id or path — `/sdd-plan oauth-login` or `/sdd-plan @context/assets/changes/oauth-login/frame.md`
For deeper analysis, try: `/sdd-plan think deeply about @context/assets/changes/oauth-login/research.md`
```
...
- **Frame brief** — path matches `context/<app>/changes/<change-id>/frame.md`, or content begins with `# Frame Brief:` / contains a `## Reframed` section.
- **Research doc** — path matches `context/<app>/changes/<change-id>/research.md`, or YAML frontmatter contains `topic:` and `researcher:` fields.
- **Existing plan** — path matches `context/<app>/changes/<change-id>/plan.md` (resume/refine mode — out of scope for this scaling logic).
...
1. **Read all mentioned files immediately and FULLY**:
   ...
   - Reference files (e.g., `context/<app>/changes/<change-id>/research.md`, `context/<app>/changes/<change-id>/frame.md`)
   - Research documents
   - Frame briefs
   - Related implementation plans
   - Any JSON/data files mentioned
   - `context/<app>/foundation/lessons.md` if present — treat its rules as priors when probing scope, edge cases, and architecture choices; rules already accepted by the team narrow which design pitfalls still need fresh questioning.
   ...
   **Example question with recommendations (non-software — content/strategy):** `Depth` is `[D]` — diagnostic about audience/scope; skip if a frame brief already settled who this is for.
   ...
   - options:
     ...
     - label: "⭐ Recommended: Hands-on with guided examples"
       description: "Concepts paired with step-by-step exercises. · Strength: Balances understanding and practice — matches the format that got highest completion rates. · Tradeoff: 2-3x more prep time per lesson; requires working example repos."
...
1. **Resolve `<app>`, resolve the change folder, then write the plan** to `context/<app>/changes/<change-id>/plan.md`.
   - **Resolve `<app>` ("ask once, store it")**: prefer inheriting it from an input path when one was given — a roadmap slice / frame / research path under `context/<app>/…` reveals the app. Otherwise resolve the workspace the way `/sdd-shape` does — auto-detect from cwd when inside a workspace dir (e.g. `packages/apps/<app>/…`); else list the workspaces (`ls -1 <workspace-parent>/`, per `/sdd-shape`'s detection order: `packages/apps/*`, `apps/*`, or `workspaces`/`pnpm-workspace.yaml` globs) and ask the user which one via the interactive-question mechanism. Confirm before creating anything.
   - **Bare-`<change-id>` resolution** (resume/refine): if the user invoked `/sdd-plan <change-id>` with no app path, glob `context/*/changes/<change-id>/` — exactly one match → use it (derive `<app>` from the path); more than one → ask which app; zero → it's a new change (resolve `<app>` as above).
   - If `context/<app>/changes/<change-id>/` already exists, use it. Otherwise derive a kebab-case `<change-id>` from the topic, `mkdir -p context/<app>/changes/<change-id>/`, and create `change.md` (mirroring `/sdd-new` semantics) with `app: <app>` in its frontmatter before writing.
   ...
   - **FALLBACK**: in a repo with no workspace layout (single-package/greenfield), skip the `<app>` segment — use flat `context/changes/<change-id>/…` and omit the `app:` field.
...
- Related research: `context/<app>/changes/<change-id>/research.md`
...
1. **Write the brief** to `context/<app>/changes/<change-id>/plan-brief.md` (sibling of `plan.md` in the same change folder).
...
> Full plan: `context/<app>/changes/<change-id>/plan.md`
> Frame brief: `context/<app>/changes/<change-id>/frame.md` (if present — omit line otherwise)
> Research: `context/<app>/changes/<change-id>/research.md` (if present — omit line otherwise)
...
1. **Confirm the plan + brief landed in the change folder**:
   ...
   - `ls context/<app>/changes/<change-id>/plan.md context/<app>/changes/<change-id>/plan-brief.md` should both exist.
...
2. **Copy quick start command to clipboard**:
   ...
   ```bash
   echo -n "/sdd-implement <change-id> phase 1" | pbcopy 2>/dev/null || echo -n "/sdd-implement <change-id> phase 1" | clip.exe 2>/dev/null || echo -n "/sdd-implement <change-id> phase 1" | xclip -selection clipboard 2>/dev/null || true
   ```

   ```powershell
   # PowerShell (Windows)
   Set-Clipboard "/sdd-implement <change-id> phase 1"
   ```
...
3. **Present both the brief and full plan**:
   ...
   📋 Brief (start here): `context/<app>/changes/<change-id>/plan-brief.md`
   📄 Full plan: `context/<app>/changes/<change-id>/plan.md`

   → /sdd-implement <change-id> phase 1 (✓ copied)
...
- **If context feels degraded during planning** — if responses become sluggish or repetitive, save the current plan draft to file and offer the user to continue in a fresh context:
  ```
  The plan draft is saved at: context/<app>/changes/<change-id>/plan.md
  Would you like to continue refining in a fresh window?
  → /sdd-plan <change-id> (✓ copied)
  ```
  This lets `/sdd-plan` reload the draft and continue iterating with full context available.
