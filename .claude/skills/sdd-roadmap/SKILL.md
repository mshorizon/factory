...
name: sdd-roadmap
description: >
  Generate context/<app>/foundation/<project-slug>/roadmap.md from a PRD as an
  ordered set of
  vertical, end-to-end slices. Use AFTER /sdd-prd (and after the tech-stack
  ...
  /sdd-plan. Trigger phrases: "write the roadmap", "generate roadmap",
  ...
  — that's /sdd-plan's job.
---

# Roadmap: Generate context/<app>/foundation/<project-slug>/roadmap.md from a PRD

This skill is the bridge between **product** (PRD) and **per-change planning** (`/sdd-plan`). Its single job: read a PRD, auto-probe the codebase baseline, **infer a decisive sequencing proposal** (main goal, north-star slice, investment areas, top blocker), surface only the genuine uncertainty the PRD can't resolve, and emit a `context/<app>/foundation/<project-slug>/roadmap.md` (a sibling of `prd.md`, inside the same project subfolder) that lists vertical, user-visible slices in dependency order — ready to feed into `/sdd-plan <change-id>`.

When the repo uses a workspace/monorepo layout, SDD artifacts are partitioned **by workspace**: `<app>` is the resolved workspace directory's name (e.g. `assets`, `campaigns`, `crm`). This skill RECEIVES a `prd.md` and infers `<app>` and `<project-slug>` from that path's `context/<app>/foundation/<project-slug>/prd.md` shape — it never re-asks for the app. App-level docs (`lessons.md`, `tech-stack.md`) live at `context/<app>/foundation/…`; per-project docs (`prd.md`, `shape-notes.md`, `roadmap.md`) live in the project subfolder. FALLBACK: repos with no workspace layout (single-package / greenfield) skip the `<app>` segment and use flat `context/foundation/<project-slug>/…`, omitting the `app:` frontmatter field.
...
It is a **decomposition + sequencing** skill, not a low-level planner. It NEVER picks frameworks, file paths, schemas, libraries, or implementation details — those belong to `/sdd-plan`. It NEVER assigns time estimates, t-shirt sizes, points, or human-calendar dates — agentic execution is non-linear and time-budgeted estimates would lie. What it DOES is: name the slices, sequence them by dependency and by stated goal, surface what's blocking, and route open questions where they can be resolved.
...
**Use when**: a `context/<app>/foundation/<project-slug>/prd.md` exists with non-trivial content (FRs and user stories populated, business logic present), AND the user wants to know what to build first / in what order. Typical triggers: just finished `/sdd-prd`, just finished bootstrap, or returning to a project and asking "what's next".

**Skip when**: the PRD is hollow (large `## Open Questions`, `# TODO: domain rule`) — point at `/sdd-prd` (or upstream `/sdd-shape`) first; a roadmap from a hollow PRD will inherit the hollowness. Also skip when the user wants to plan a _single_ change in detail — that's `/sdd-plan`. The roadmap is plural; the plan is singular.

## Relationship to other skills

- `/sdd-shape` and `/sdd-prd` — produce the upstream PRD this skill consumes. If `shape-notes.md` carries a `## Forward: technical-roadmap` block (where shape parks roadmap-bound content), this skill lifts it.
- `sdd-tech-stack-selector` — runs between `/sdd-prd` and this skill in the bootstrap chain. If `context/<app>/foundation/tech-stack.md` exists (app-level), this skill reads it as input to derive `## Foundations` (auth scaffold, deploy skeleton, observability — anything the tech-stack-selection step implied) and to short-circuit baseline probes for layers already declared.
- `/sdd-plan` — downstream consumer. The user picks a roadmap item and invokes `/sdd-plan <change-id>`; that skill creates the change folder `context/<app>/changes/<change-id>/` (resolving its own `<app>`) and produces a detailed plan. The roadmap does NOT pre-create change folders; one slice can spawn multiple changes when `/sdd-plan` discovers that the item is still too broad.
- `/sdd-implement` — further downstream. The intermediate lifecycle states (`status: planning`, `in-progress`) are defined here but **not yet wired** in `/sdd-plan` and `/sdd-implement`; today this skill writes only `proposed` / `ready` / `blocked`. Future work will wire the intermediate states.
- `/sdd-archive` — closes the loop at the end. When a change whose `Change ID` matches a roadmap item is archived, `/sdd-archive` flips that item's `Status` to `done` (in `## At a glance` and in the item's body block) and appends an entry to `## Done`. This skill never pre-populates `## Done`; `/sdd-archive` is its sole writer.
- `/sdd-frame`, `/sdd-research` — orthogonal. They operate on a single change, not the roadmap.

## Initial Response

When this skill is invoked:

1. **If a path argument was provided** (e.g. `/sdd-roadmap @path/to/prd.md`), capture it as the PRD path. Infer `<app>` and `<project-slug>` from the path's `context/<app>/foundation/<project-slug>/prd.md` shape (the first segment after `context/` is `<app>`; the segment after `foundation/` is `<project-slug>`). Proceed to Step 1.
2. **If no argument was provided**, discover the PRD via the glob `context/*/foundation/*/prd.md` (see Step 1) and proceed to Step 1. Do not prompt yet — Step 1 handles the discovery and missing-input cases.
...
- Otherwise discover the PRD by globbing `context/*/foundation/*/prd.md` across all apps:

  ```bash
  ls context/*/foundation/*/prd.md 2>/dev/null
  ```

  - **Exactly 1 match** → use it.
  - **Multiple matches** → ask the user which app/project to roadmap (one structured question listing each `<app>/<project-slug>` candidate).
  - **0 matches** → fall through to the missing-input branch below.

  (FALLBACK: a repo with no workspace layout (single-package / greenfield) uses flat `context/foundation/*/prd.md` instead.)

Derive `<app>` and `<project-slug>` from the chosen PRD path (`context/<app>/foundation/<project-slug>/prd.md`); the roadmap is written into that same project folder. The skill never re-asks for `<app>` once inferred.
...
If no PRD is found (0 glob matches, or the passed path is missing), ask with the selected interactive-question tool:
...
- question: "No PRD found at `<resolved-path>`. How would you like to proceed?"
  header: "Input?"
  options:
  - label: "Run /sdd-prd first (Recommended)"
    description: "Stop here. Run /sdd-prd to produce prd.md, then re-invoke /sdd-roadmap."
...
On "Run /sdd-prd first": print the redirect message and STOP.

### Step 2: Read supplementary inputs (best effort)

Read these if they exist; otherwise note their absence and continue. Use the `<app>` and `<project-slug>` inferred in Step 1. Per-project docs live in the same project subfolder as the PRD; app-level docs live one level up at `context/<app>/foundation/`:

- `context/<app>/foundation/<project-slug>/shape-notes.md` — look for a `## Forward: technical-roadmap` section. If present, lift its bullets verbatim as candidate roadmap inputs (the user already parked them there during shaping).
- `context/<app>/foundation/tech-stack.md` (app-level) — informs the `## Foundations` section AND short-circuits baseline probes (a layer already declared here is reported as "per tech-stack.md" without re-probing).
- `context/<app>/foundation/<project-slug>/roadmap.md` — if it already exists, hold it for Step 9 (collision handling). Do NOT mutate it yet.
- `context/<app>/foundation/lessons.md` (app-level) — if present, scan for any rules that touch ordering or readiness (e.g., "always ship the riskiest slice first"). Treat as priors, not gospel.
...
- question: "How would you like to proceed?"
  ...
  - label: "Firm up PRD first (Recommended)"
    description: "Stop here. Resolve PRD's Open Questions / TODOs, then re-invoke /sdd-roadmap."
...
- Is small enough that one `/sdd-plan` invocation produces a tractable plan, but big enough that the slice is meaningful on its own (a slice is generally one US-NN, occasionally two when they're tightly coupled — e.g., "create" and "list" of the same entity).
...
Each `F-NN` and `S-NN` also gets a stable **Change ID** in kebab-case. The Change ID is the bridge into `/sdd-plan` and, later, a backlog item in Jira/Linear. Prefer concise, outcome-oriented names such as `first-gated-generation`, `minimal-auth-for-generation`, or `srs-review-session`.
...
- **Unknowns** (questions to research) — things the roadmap can't answer that `/sdd-plan` shouldn't try to either. Each unknown carries: question, owner, blocking-status (yes/no — is planning blocked until this resolves?). The Step 5 "Decisions" #1-blocker answer feeds these.

A slice with `Status: blocked` exists when at least one Unknown has `Block: yes`. The roadmap's job is to surface these so the user can resolve them before `/sdd-plan` is wasted on a slice that can't be planned.
...
Use this exact template (section names are the contract; downstream tooling and `/sdd-plan` may grep for them):

```markdown
---
project: <from PRD frontmatter>
app: <app> # carried from the PRD frontmatter; OMIT in repos with no workspace layout
version: 1
...
> Derived from `context/<app>/foundation/<project-slug>/prd.md` (v<N>) + auto-researched codebase baseline.
...
| Roadmap ID | Change ID              | Suggested issue title         | Ready for `/sdd-plan` | Notes                       |
| ---------- | ---------------------- | ----------------------------- | --------------------- | --------------------------- |
| F-01       | <kebab-case-change-id> | <issue title for Jira/Linear> | no                    | <why or `—`>                |
| S-01       | <kebab-case-change-id> | <issue title for Jira/Linear> | yes                   | Run `/sdd-plan <change-id>` |
...
(Empty on first generation. `/sdd-archive` appends an entry here — and flips that item's `Status` to `done` — when a change whose `Change ID` matches the item is archived. Do NOT pre-populate. Format:)

- **<Slice ID>: <Outcome>** — Archived <YYYY-MM-DD> → `context/<app>/archive/<YYYY-MM-DD-change-id>/`. Lesson: <pointer to lessons.md if any, or `—`>.
```
...
- **Change ID** is kebab-case, stable, and suitable for `context/<app>/changes/<change-id>/` (the folder `/sdd-plan` will later create, resolving its own `<app>`). Do not use `F-01` / `S-01` as the change id; those are roadmap-local order IDs.
...
- **Status** lifecycle: `proposed` (default on first generation) | `ready` (Prerequisites all met, no blocking unknowns — `/sdd-plan` can run) | `planning` | `in-progress` | `done` | `blocked` (one or more unknowns with `Block: yes`). Today this skill emits only `proposed`, `ready`, and `blocked`; `/sdd-archive` flips an item to `done` when its change archives. `planning` and `in-progress` are reserved for future `/sdd-plan` / `/sdd-implement` wiring.
...
**No time units. No estimates. No complexity scores.** No "Day 1", no "Week 2", no "small/medium/large", no story points. Order is encoded in Prerequisites; pacing is encoded in Blockers and Unknowns. If you find yourself wanting to write "this should take a few hours" — stop. That's `/sdd-plan`'s downstream territory, and even there it's about scope clarity, not calendar prediction.

### Step 8: Self-review

Before any disk write, verify the in-memory roadmap:

1. **Frontmatter** — all 8 core keys present (`project`, `version`, `status`, `created`, `updated`, `prd_version`, `main_goal`, `top_blocker`), plus `app` (carried from the PRD frontmatter) in workspace repos — omitted only in repos with no workspace layout.
...
The roadmap target is `context/<app>/foundation/<project-slug>/roadmap.md` (the project subfolder the PRD lives in).

```bash
test -f context/<app>/foundation/<project-slug>/roadmap.md
```

If the file does not exist, write to `context/<app>/foundation/<project-slug>/roadmap.md` and proceed to Step 10.
...
- question: "context/<app>/foundation/<project-slug>/roadmap.md already exists. How would you like to proceed?"
  header: "Collision"
  options:
  - label: "Archive and replace (Recommended)"
    description: "Move existing to context/<app>/foundation/<project-slug>/archive/<today>-roadmap.md, then write the new roadmap. History preserved per foundation README convention."
...
On "Archive and replace": create `context/<app>/foundation/<project-slug>/archive/` if missing, move the existing file to `context/<app>/foundation/<project-slug>/archive/<today>-roadmap.md` (use today's date in `YYYY-MM-DD`), then write the new content. If a file already exists at that archive path (regenerated twice in one day), append `-2`, `-3`, etc.
...
═══════════════════════════════════════════════════════════
  ...
  Project:           <project>
  Path:              context/<app>/foundation/<project-slug>/roadmap.md
...
```
► **Your next move:** `/sdd-plan <change-id>` on **<Roadmap ID>: <Outcome>**.
...
► **No planning move is available yet.** Every slice is blocked.
  ...
  most opens the roadmap. Resolve it, then re-invoke `/sdd-roadmap` to
  re-recommend.
```
...
4. **No low-level technical details.** No frameworks named (those live in `tech-stack.md`), no file paths, no schema definitions, no code, no library choices. If you find yourself writing those, you've crossed into `/sdd-plan`'s territory — stop and let `/sdd-plan` do its job downstream.
...
8. **Foundation-doc convention.** `roadmap.md` is a per-project foundation doc per the `context/` README. Default collision handling is archive-then-replace (history goes to `context/<app>/foundation/<project-slug>/archive/<today>-roadmap.md`); surgical refinement is out of scope for this skill (edit by hand if you need it).

9. **Universal language only.** No cohort / certification references in any user-facing output or any artifact written to disk. The skill is a generic roadmap generator.

10. **Never chain automatically.** Step 10 is an announcement, not an invocation. The user picks when (and which) slice to feed to `/sdd-plan`. Auto-chaining would skip the human's review of the generated roadmap.
...
- This skill is a **document generator**. Output is `context/<app>/foundation/<project-slug>/roadmap.md`, period. Per-change planning lives downstream in `/sdd-plan`.
...
- The `## Done` section is empty on first generation. It exists so `/sdd-archive` has a stable place to record closed items — when a change whose `Change ID` matches a roadmap item is archived, `/sdd-archive` flips that item to `Status: done` and appends a `## Done` entry. Do NOT pre-populate it.
- When the skill regenerates an existing roadmap, the previous file moves to `context/<app>/foundation/<project-slug>/archive/<today>-roadmap.md`. Reading the diff between the archived version and the new one is the cleanest way to see what changed in the project's understanding — that's the affordance the foundation-doc convention is designed for.
- Lifecycle status fields `planning` and `in-progress` are reserved — today this skill emits only `proposed` / `ready` / `blocked`, and `/sdd-archive` flips an item to `done` on archive. Wiring `/sdd-plan` and `/sdd-implement` to flip `planning` / `in-progress` is future work.
