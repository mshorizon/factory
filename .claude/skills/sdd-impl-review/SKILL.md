...
2. Argument is a bare `<change-id>` → resolve the app by globbing `context/*/changes/<change-id>/` (look for `plan.md`): exactly one match → use it and derive `<app>` from the resolved path; more than one match → ask which app; zero matches → the not-found branch below. Then fresh review on `context/<app>/changes/<change-id>/plan.md`. (Fallback: repos with no workspace layout use flat `context/changes/<change-id>/plan.md`.)
3. Plan path provided (e.g. `@context/<app>/changes/<change-id>/plan.md`) → fresh review on that plan; derive `<app>` from the path.
...
This skill receives a plan path or `<change-id> [phase N]` — it does NOT ask for the app; the app is resolved from the path/glob above ("ask once, store it").

If the resolved plan path matches `context/*/archive/…`, refuse: print "This change is archived. Reviews are not appended to archived plans." and STOP.
...
2. **Read `context/<app>/foundation/lessons.md` if present** and use accepted rules as priors when scanning for findings — a deviation that violates a known recurring rule is a stronger signal than a generic style nit.
...
options:
  ...
  - label: "Save report & triage later"
    description: "Save the full report. Resume with /sdd-impl-review <report-path>."
...
Save to `context/<app>/changes/<change-id>/reviews/impl-review.md` (or `context/<app>/changes/<change-id>/reviews/impl-review-phase-N.md` for a phase-scoped review). Update `change.md`: set `status: impl_reviewed` and `updated: <today>`. If the user opts to triage, queue any "fix in plan/code" follow-ups into `context/<app>/changes/<change-id>/follow-ups/review-fixes.md`.
...
"Save & triage later" → save, print the path, remind them to run `/sdd-impl-review <saved-report-path>`.
...
options:
  ...
  - label: "Record as lesson"
    description: "Save as a recurring project rule via /sdd-lesson."
multiSelect: false
```
...
- **Record as lesson**: pre-fill four lessons-entry fields directly from the finding — `Context` from the finding's Location, `Problem` from the finding's Detail, `Rule` and `Applies to` left as empty placeholders for the user to fill. Show the proposed entry as a complete markdown block and ask the user to edit / confirm via Ask the user: ("Approve this entry?" / "Edit before saving" / "Cancel"). On confirm, append the entry as a new H2 section to `context/<app>/foundation/lessons.md` — if the file does not exist, create it first with this canonical 5-line header (no separate template file; the header is embedded inline here):

  ```
  # Lessons Learned

  > Append-only register of recurring rules and patterns. Re-read at start by /sdd-frame, /sdd-research, /sdd-plan, /sdd-plan-review, /sdd-implement, /sdd-impl-review.
