...
description: >
  ...
  Use AFTER /sdd-prd (brownfield), BEFORE /sdd-health-check.
---

# Stack Assess: Evaluate an Existing Stack for Agent-Friendliness

This skill is the brownfield counterpart to `/sdd-tech-stack-selector`. Where tech-stack-selector helps greenfield users **choose** a stack, stack-assess helps brownfield users **evaluate** theirs. It reuses the same four agent-friendly quality gates (`references/agent-friendly-criteria.md`) but applies them as an evaluation lens rather than a selection filter.

The skill sits in the brownfield chain: `/sdd-shape → /sdd-prd → /sdd-stack-assess → /sdd-health-check`. Its single job: evaluate the existing stack against the quality gates and produce a structured assessment with concrete compensation strategies.
...
**Skip when**: the user is starting a new project from scratch — redirect to `/sdd-tech-stack-selector`. Skip also when the user just wants a dependency audit or security scan without the quality-gate framing — that is `/sdd-health-check` territory.

## Relationship to other skills

- `/sdd-shape` — upstream. Produces `shape-notes.md` with `context_type: brownfield`.
- `/sdd-prd` — upstream. Produces `context/foundation/prd.md` (brownfield template). Optional input — the skill can run without a PRD.
- `/sdd-tech-stack-selector` — greenfield parallel. Same quality gates, different job (selection vs evaluation).
- `/sdd-health-check` — downstream consumer. Reads `context/foundation/stack-assessment.md` to focus health checks on identified gaps.
...
1. **If a path argument was provided** (e.g. `/sdd-stack-assess @context/foundation/prd.md`), strip a leading `@` if present and use the path as the PRD location for this run. The PRD is optional context, not a precondition — the skill runs without it.
...
```
No project markers found in the current directory. /sdd-stack-assess requires an existing codebase.
If you're starting from scratch, use /sdd-tech-stack-selector instead.
```
...
<overall verdict, key strengths, key gaps, and recommended next step (/sdd-health-check)>
```
...
```bash
echo -n "/sdd-health-check" | pbcopy 2>/dev/null || echo -n "/sdd-health-check" | clip.exe 2>/dev/null || echo -n "/sdd-health-check" | xclip -selection clipboard 2>/dev/null || true
```

```powershell
# PowerShell (Windows)
Set-Clipboard "/sdd-health-check"
```
...
═══════════════════════════════════════════════════════════
  ...
  ► Next:        /sdd-health-check  (✓ copied to clipboard)
═══════════════════════════════════════════════════════════
```

STOP. Do not chain into `/sdd-health-check` automatically — the user runs it when ready.
