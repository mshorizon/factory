---
name: sdd-implement
description: Implement technical plans from context/<app>/changes/<change-id>/plan.md with verification
---

# Implement Plan

You are tasked with implementing an approved technical plan from `context/<app>/changes/<change-id>/plan.md`. These plans contain phases with specific changes and a canonical `## Progress` section at the bottom that drives execution state (see `references/progress-format.md`).
...
1. **Resolve the plan**:
   - If invoked as `/sdd-implement <change-id> [phase N]` with a bare change-id, resolve by globbing `context/*/changes/<change-id>/plan.md`: exactly one match → use it; more than one match → ask which app; zero matches → fall through to the "no plan found" branch below. Derive `<app>` from the resolved path. (An explicit `<app> <change-id>` invocation also works — use it directly.)
   - If invoked with `@context/<app>/changes/<change-id>/plan.md` or a full path, accept it and derive `<app>` from the path.
   - **Fallback (repos with no workspace layout — single-package/greenfield)**: use the flat layout `context/changes/<change-id>/plan.md`.
   - **Refuse if the resolved path matches `context/*/archive/…`** — print "This change is archived. Open a new change with `/sdd-new` instead." and STOP.
...
1. A change-id (e.g., `/sdd-implement oauth-login phase 1`), or
2. A full path (e.g., `@context/<app>/changes/oauth-login/plan.md`).
...
- Read `context/<app>/foundation/lessons.md` if present and internalize each entry before starting any phase — these are the team's accepted recurring rules and must shape every implementation choice you make in this run.
...
- The set always contains `context/<app>/changes/<change-id>/plan.md` because each phase produces at least one modification to its `## Progress` section. Add it on entry to a phase even before any checkboxes flip.
- **Phase 1 bootstrap**: on the first phase of a change, also seed the touched-file set with all untracked or modified files inside `context/<app>/changes/<change-id>/` — typically `change.md`, `research.md`, `plan.md`, and any other context files created during planning. These files are part of the change and should land in the first commit rather than being left as untracked stragglers.
...
- **Run the phase-end commit ritual**: After all automated checks pass for the phase, walk through this sequenced ritual to author one Conventional-Commits commit and write the closing short SHA back into every Progress row flipped during the phase.
  ...
  2. **Compute the staging set.** Take the touched-file set maintained during the phase (see "Tracking files touched during a phase" above) and union it with `{context/<app>/changes/<change-id>/plan.md}`. The plan file is always staged because each phase produces at least one modification to its `## Progress` section.
  ...
  9. **Write the SHA back into Progress.** For every Progress row flipped during this phase, run a targeted file edit:
     ...
     Skip rows that already carry a SHA suffix (resume safety: if the ritual is re-entered after a partial run, do not double-append). If `SHA=""`, skip the append entirely — the rows stay SHA-less and `/sdd-archive` will surface them as informational warnings under its missing-SHA soft-warning check.
...
- **Next phase decision**: If there is a next phase, help the user decide whether to continue or start fresh.
  ...
  - "Review this phase first" (description: "Run /sdd-impl-review to verify implementation against the plan before proceeding.")

  **If user chooses to review**: Run `/sdd-impl-review @[path-to-plan] phase [N]` to review the just-completed phase. After the review completes, re-present the continue/clear decision (without the review option this time).
  ...
  **If user chooses to clear**: Copy the resume command to clipboard and display it. Keep the short `/sdd-implement <change-id> phase N` form — on re-entry the skill re-resolves `<app>` from the change-id via the glob, so do not bake the app into the clipboard command:

  1. Copy:

     ```bash
     echo -n "/sdd-implement <change-id> phase [next-phase-number]" | pbcopy 2>/dev/null || echo -n "/sdd-implement <change-id> phase [next-phase-number]" | clip.exe 2>/dev/null || echo -n "/sdd-implement <change-id> phase [next-phase-number]" | xclip -selection clipboard 2>/dev/null || true
     ```

     ```powershell
     # PowerShell (Windows)
     Set-Clipboard "/sdd-implement <change-id> phase [next-phase-number]"
     ```

  2. Display:
     ```
     → /sdd-implement <change-id> phase [next-phase-number] (✓ copied)
     ```
...
Empty-diff phases (manual-verification-only or no-op adapted phases) commit nothing and leave their rows SHA-less; `/sdd-archive` will surface them as informational warnings under its missing-SHA soft-warning check. This is intentional — not every phase produces code.
...
1. **Defensive pending-items surface.** Re-scan the entire `## Progress` section one last time for any `- [ ]` rows. Under normal flow this is a no-op — the trigger condition for "After all phases" is already "every `- [ ]` is `- [x]`", so the surface should find nothing. It exists to make any unexpected stragglers explicit rather than silently lost (e.g., if a partial run, a manual edit, or a resume path bypassed the trigger). If the count is non-zero, list each row as `<phase>.<index> <title>` grouped by Automated vs Manual subsection in document order, then ask the user:
   ...
   - "Proceed to epilogue" (description: "Flip status: implemented and run the epilogue commit anyway. Stragglers will surface as warnings under /sdd-archive.")
...
2. Update `change.md`: set `status: implemented`, `updated: <today>`. (Do NOT set `archived_at` — that belongs to `/sdd-archive`.)
3. Do NOT write any HTML comment progress marker at the bottom of the plan.
4. **Run the epilogue commit.** The final phase's commit cannot contain its own SHA (chicken-and-egg), so the SHA write-back into the final phase's Progress rows plus the `change.md` status flip both sit dirty in the working tree after the final phase ritual returns. Author one closing commit to land them — otherwise `/sdd-archive`'s hard-refusal gate (uncommitted paths inside the change folder) will block. Steps:
   1. Stage exactly `context/<app>/changes/<change-id>/plan.md` and `context/<app>/changes/<change-id>/change.md` (explicit paths, no `git add -A`).
...
- "Run full review (/sdd-impl-review)" (description: "Comprehensive review of all phases against the plan. Catches cross-phase issues.")
- "Skip review — I'm satisfied" (description: "No review needed. Mark the plan as done.")

If user chooses review → run `/sdd-impl-review <change-id>` (no phase number = full plan review).
