...
export const onRequest = defineMiddleware(async (context, next) => {
  ...
  // Start health check cron (idempotent, runs once)
  initHealthCron();
  ...
  const log = createRequestLogger(request, businessId);
  locals.logger = log;
  ...
  try {
    ...
    if (response.status >= 500) {
      log.error({ status: response.status, duration }, "Request completed with server error");
      track5xxError(businessId);
    } else if (response.status >= 400) {
      log.warn({ status: response.status, duration }, "Request completed with client error");
    } else {
    ...
    const duration = Date.now() - start;
    log.error({ err: error, duration }, "Unhandled error in request");
