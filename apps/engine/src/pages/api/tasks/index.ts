...
export const POST: APIRoute = async ({ request, cookies, locals }) => {
  ...
  try {
    ...
    const { domain, template, location, description } = body ?? {};

    if (!domain || !template || !location || !description) {
      return new Response(
        JSON.stringify({
          error: "Missing required fields (domain, template, location, description)",
        }),
